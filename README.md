...
- **Paste from clipboard** — `Cmd/Ctrl+V` anywhere on `/ipad` (or the **Paste** toolbar button for touch) drops the clipboard contents into the note. Text is word-wrapped at the page margins; images are scaled to fit and the canvas auto-grows to fit pasted content. If a pasted URL is blocked by the source site (403 / CORS / hotlink protection), it falls back to plain text with a non-blocking toast explaining the issue instead of a raw error banner. The image-URL detector is strict — only URLs ending in a known image extension (`.png`, `.jpg`, `.gif`, `.webp`, `.bmp`, `.svg`, `.avif`, `.heic`, `.heif`, `.tiff`) or `data:image/…` URIs are fetched as images; share links (e.g. `chatgpt.com/share/…`) are pasted as text.
